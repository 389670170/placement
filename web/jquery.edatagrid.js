...
(function($){
	function buildGrid(target){
			...
			onAfterEdit: function(index, row){
				...
				if (url){
                    ...
                    for(var field in editableFields) {
                        commitRow[field] = row[field];
                    }
					$.get(url, commitRow, function(data){
                        if(data.code == 0) {
                            // 更新数据
                            if(data.data.fight_force) {
                                row.fight_force = data.data.fight_force; 
                            }
                            ...
                            $.messager.alert('错误', '更新失败 desc:' + data.desc, 'error');
		...
		function focusEditor(field){
            ...
            var allEditors = $(target).datagrid('getEditors', opts.editIndex);
            for(var i = 0; i < allEditors.length; i++) {
                editableFields[allEditors[i].field] = 1;
            }
            editableFields[opts.idField] = 1;
		}
					...
					if (r){
						...
						if (row.isNewRecord){
							...
							if (opts.destroyUrl){
								...
								$.get(opts.destroyUrl, {id:idValue}, function(data){
                                    ...
                                    if(resp.code == 0) {
                                        ...
                                        $.messager.alert('错误', '删除失败 desc:' + resp.desc, 'error');
                                    }
								});
							} else {
								dg.datagrid('cancelEdit', index);
