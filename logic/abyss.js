...
function is_valid_progress(player, progress) {
    ...
    if (!diff_max_regin) {
        ...
        for (var tKey in gConfAbyssCustom) {
            var diff = ~~(tKey / 100000);
            var tRegion = ~~((tKey % 100000) / 100);
            diff_max_regin[diff] = gConfAbyssCustom[diff] > tRegion ? gConfAbyssCustom[diff] : tRegion;
        }
    }
    ...
    var user = player.user;
    if (gConfAbyssCustom[progress] && gConfAbyssCustom[progress].challengeLimit == "level" && gConfAbyssCustom[progress].target > user.status.level) {
        return false;
    }
    ...
    if (region == 1 && little_progress == 1) {
        if (diff == 1) {                                                                    // 第一关 只要功能开启就可以打
            ...
            return abyss_info.progress[tProgress]
        }
    }
    else if (little_progress == 0) {
        var tTotalStar = 0;
        for (var i = 0; i < MAX_LITTLE_PROGRESS; i++) {
            tTotalStar += abyss_info.progress[(diff * 100000) + (region * 100) + i + 1];
        }

        return (tTotalStar == (MAX_LITTLE_PROGRESS * 3));                                    // 旋涡关卡 直接判断上一个关卡是否全3星
    }
    else if (little_progress == 1) {
        return abyss_info.progress[(diff * 100000) + ((region - 1) * 100) + MAX_LITTLE_PROGRESS];              // 一般关卡 判断上一区域的最后一关是否通关
    }
    else {
        return (abyss_info.progress[(diff * 100000) + (region * 100) + (little_progress - 1)] > 0);            // 不是旋涡关卡 直接判断上一个关卡是否已经通过
    }
    return false;
};
...
exports.fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        abyss_info.progress[progress] = (abyss_info.progress[progress] > star) ? abyss_info.progress[progress] : star;
        player.markDirty("abyss");
        resp.data.awards = player.addAwards(gConfAbyssCustom[progress].award, req.mod, req.act);
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        for (var i = start_progress; i < (start_progress + MAX_LITTLE_PROGRESS); i++) {
            tTotalStarCount = tTotalStarCount + abyss_info.progress[i];
        }
