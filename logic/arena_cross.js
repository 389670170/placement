...
exports.get = function (player, req, resp, onHandled) {
    ...
    do {
        if (!isModuleOpen_new(player, 'arena12')) {
        ...
        function callback(player, resp) {
            ...
            if (resp.code == 0) {
                ...
                if (!arena.max_cross_rank || resp.data.rank < arena.max_cross_rank) {
                    arena.max_cross_rank = resp.data.rank;
                    player.markDirty('arena.max_cross_rank');
                }

                player.memData.status = 'idle';

                resp.data.max_rank = arena.max_cross_rank || 0;
                resp.data.award_got = arena.award_got;
                resp.data.count = arena.count_cross;
        ...
        requestArenaServer(req, resp, function () {
            resp.data.my_type = resp.data.my_type;
            resp.data.my_rank = resp.data.my_rank;
...
exports.challenge = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (user.arena.count_cross >= gConfGlobalNew.arenaMaxCount) {
            if (!player.checkCosts(cost)) {
                resp.code = 1; resp.desc = 'has no count_cross'; break;
            }
        }
...
exports.fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        function callback(player, resp) {
            ...
            if (resp.code == 0) {
                if (user.arena.count_cross < gConfGlobalNew.arenaMaxCount) {
                    user.arena.count_cross++;
                    player.markDirty('arena.count_cross');
                } else {
                ...
                if (star > 0) {
                    ...
                    if (diff > 0) {
                        user.arena.max_cross_rank = rank;
                        player.markDirty('arena.max_cross_rank');
                    }
...
exports.rank_list = function (player, req, resp, onHandled) {
    ...
    req.mod = "arena";
    requestArenaServer(req, resp, () => {
        resp.data.rank_list = resp.data.rank_list || [];

        var selfIndex = resp.data.self_rank;
        delete resp.data.self_rank;
        var selfRankData = resp.data.self_rank_data;
        delete resp.data.self_rank_data;
        resp.data.self = resp.data.self || {
            uid: user._id,
            un: user.info.un,
            ...
            rank: selfIndex >= 0 ? selfIndex + 1 : 0,
        };
        if (selfRankData) {
            resp.data.self.rank_type = selfRankData.type;
            resp.data.self.rank_index = selfRankData.rank;
        }

        for (var i = 0; i < resp.data.rank_list.length; i++) {
            if (i != 0) { continue; }                               // 只有第一名 需要显示军团名

            var info = resp.data.rank_list[i];
            var tLegionReq = {
                act: "get_user_legion_info",
                mod: "new_legion",
                args: {}
            }
            tLegionReq.uid = info._id;
            tLegionReq.args.idx = i;
            var new_resp = {};
            requestWorld(tLegionReq, new_resp, () => {
                if (new_resp.data.legion_data) {
                    resp.data.rank_list[new_resp.data.idx].legionName = new_resp.data.legion_data.name;
                }
                onHandled();
            });
            break;
...
exports.get_achievement_awards = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 检查成就是否达成
        var bFinish = false;
        if (arena.max_cross_rank <= conf.count) {
            bFinish = true;
        }
